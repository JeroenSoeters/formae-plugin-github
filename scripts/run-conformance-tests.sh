...
# =============================================================================
# Multiple agents may run conformance tests concurrently against the same
# GitHub test repo. Use flock to serialize runs. The lock is automatically
# released when the process exits (including crashes/signals).
LOCKFILE="/tmp/formae-github-conformance.lock"
exec 9>"${LOCKFILE}"
if ! flock -n 9; then
    echo "Another conformance test is running (lockfile: ${LOCKFILE}). Waiting..."
    flock 9
fi
echo "Acquired conformance test lock (${LOCKFILE})"
